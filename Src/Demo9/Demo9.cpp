...
Demo9::Demo9(uint width, uint height):DemoBase(width,height){
    // 设置窗体回调函数
    ...
    m_rect.init("src/Demo9/cube.vs", "src/Demo9/cube.fs", vecVert, vecId);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);  // 1. 设置顶点属性指针
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));
    glEnableVertexAttribArray(1);  // 1. 设置顶点属性指针
    
    m_lightCube.init("src/Demo9/light.vs", "src/Demo9/light.fs", vecVert, vecId);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);  // 1. 设置顶点属性指针
...
void Demo9::render(){
    ...
    m_rect.shader.setVec3("material.ambient",  vec3(1.0f, 0.5f, 0.31f));
    m_rect.shader.setVec3("material.diffuse",  vec3(1.0f, 0.5f, 0.31f));
...
uint Demo9::generateTexture(string imgPath, uint filterType, uint repeatType){
    int width, height;
    uint tex;
    // 加载图片
