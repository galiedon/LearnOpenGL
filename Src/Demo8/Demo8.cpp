...
static float cube[] = {    -0.5f, -0.5f, -0.5f,  0.0f,  0.0f, -1.0f,
...
Demo8::Demo8(uint width, uint height):DemoBase(width,height){
    // 设置窗体回调函数
    ...
    m_rect.init("src/Demo8/cube.vs", "src/Demo8/cube.fs", vecVert, vecId);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);  // 1. 设置顶点属性指针
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)(3 * sizeof(float)));
    glEnableVertexAttribArray(1);  // 1. 设置顶点属性指针
    
    m_lightCube.init("src/Demo8/light.vs", "src/Demo8/light.fs", vecVert, vecId);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);  // 1. 设置顶点属性指针
...
void Demo8::render(){
    ...
    m_rect.shader.setVec3("lightPos", m_lightPos);
...
uint Demo8::generateTexture(string imgPath, uint filterType, uint repeatType){
    int width, height;
    uint tex;
    // 加载图片
